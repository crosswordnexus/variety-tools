...
function processWordList(fileContents, minScore) {
  ...
  lines.forEach(line => {
    let ws = line.split(';');
    if (ws.length > 1) {
      let word = ws[0].toLowerCase();
      let score = parseInt(ws[1]);
      let myLen = word.length;
      if (myLen >= 6 && score >= minScore) {
        if (!window.WORDS[myLen]) window.WORDS[myLen] = new Set();
        window.WORDS[myLen].add(word);
      }
    }
  });
  return;
